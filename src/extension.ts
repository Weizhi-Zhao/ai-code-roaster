...
class AICodeRoasterViewProvider implements vscode.WebviewViewProvider {
  ...
  // Public method to refresh the webview
  async refresh() {
    if (this._view) {
      await this.onWebviewShown();
    }
  }
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const fileChangeDisposable = vscode.window.onDidChangeActiveTextEditor(async (editor) => {
    // Only trigger if sidebar is visible
    if (provider.isVisible && editor?.document) {
